...
def catalog(version: int = CURRENT_ASSESSMENT_VERSION) -> list[AssessmentQuestion]:
    return list(ASSESSMENT_QUESTION_CATALOG.get(version))


def visible_ids(
    answer_values: dict[str, AssessmentAnswer], version: int = CURRENT_ASSESSMENT_VERSION
) -> list[str]:
    return [
        question.id
        for question in AdaptiveBranchingEngine().visible_questions(catalog(version), answer_values)
    ]
...
def test_catalog_is_versioned_grouped_and_dependency_safe() -> None:
    ...
    assert QuestionCategory.NUTRITION not in question_catalog.grouped(CURRENT_ASSESSMENT_VERSION)
...
def test_gender_age_and_experience_branches_are_deterministic() -> None:
    male_branch = visible_ids(answers(user_gender="male"), version=1)
    non_male_branch = visible_ids(answers(user_gender="female"), version=1)

    assert "male_health_context" in male_branch
    assert "male_health_context" not in non_male_branch
    assert "advanced_programming_style" not in visible_ids(
        answers(age=15, experience="advanced"), version=1
    )
    assert "advanced_programming_style" in visible_ids(
        answers(age=16, experience="advanced"), version=1
    )
    assert "advanced_programming_style" not in visible_ids(
        answers(age=30, experience="beginner"), version=1
    )


def test_goal_injury_and_location_answers_change_visibility_and_priority() -> None:
    fat_loss_ids = visible_ids(answers(primary_goal="fat_loss"), version=1)
    assert fat_loss_ids.index("nutrition_pattern") < fat_loss_ids.index("has_injury")
    assert "target_weight" in fat_loss_ids

    knee_ids = visible_ids(answers(has_injury=True, injury_area=["knee"]), version=1)
    assert "knee_injury_details" in knee_ids
    assert "knee_injury_details" not in visible_ids(
        answers(has_injury=True, injury_area=["ankle"]), version=1
    )

    home_ids = visible_ids(answers(home_training=True), version=1)
    gym_ids = visible_ids(answers(home_training=False), version=1)
...
def test_essential_setup_defers_feature_specific_questions() -> None:
    ids = {question.id for question in catalog()}
    assert "nutrition_pattern" not in ids
    assert "sleep_hours" not in ids
    assert "sports" not in ids
    assert "equipment_available" in visible_ids(answers(home_training=True))
    assert "commercial_gym_equipment" in visible_ids(answers(home_training=False))
